...
/**
 * A common JMX utility. strongly recommending to connect/close each time
 * instead of keeping a long time of connection. 
 */
public class JMXUtil {
	 ...
	 * 2014-6-27 下午4:11:18 created by David Liu
	 */
	public static MBeanServerConnection conn(String ip, int port,String username,String password) {
...
//	          map.put(Context.SECURITY_CREDENTIALS, password);
	...
	/**
	 * Get all MBeans
	 * 2014-6-27 下午3:07:33 created by David Liu
	 */
	...
	/**
	 * Convert TabularDataSupport to String
	 * 2014-6-27 下午3:07:53 created by David Liu
	 */
	...
	/**
	 * Convert CompositeData to String
	 * 2014-6-27 下午3:08:04 created by David Liu
	 */
	...
	public static void main(String[] args) throws Exception {
		FileOutputStream fos=new FileOutputStream("Kakfajmx-47.txt");
		MBeanServerConnection conn = conn("10.0.1.47", 9999, null, null);
		...
		for (ObjectName on : ons) {
			...
			for (MBeanAttributeInfo mbai : mbAttributes) {
				sb.append("\tattributeName="+mbai.getName()+"\ttype="+mbai.getType()+"\r\n");
			}
		}
	...
	public static String stringMBeanInfo(MBeanServerConnection conn,
		...
		for (MBeanAttributeInfo mbai : mbAttributes) {
			...
			String tmp1="\r\nObjectName="+objName+"\tAttributeName="+attrName+"\twritable="+mbai.isWritable()+"\treadable="+mbai.isReadable();
...
//			System.out.println("\t"+attrValue);
	...
	/**
	 * Given an ObjectName and its attribute name, retrieve that attribute's value
	 * 2014-6-27 下午3:08:35 created by David Liu
	 */
...
//					}
				...
				if(classType.startsWith("Ljava.lang") && classType.length()>2){
					/**
					 * If coming dowon to here, that means some types are not covered,
					 * then needs to add more logics.
					 */
